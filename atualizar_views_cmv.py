"""
Atualiza as views CMV para incluir dados de 2025-2026
"""
import os
...
def atualizar_views():
    print("=" * 60)
    print("ATUALIZANDO VIEWS CMV - Range 2025 a 2026")
    print("=" * 60)
    print()
    ...
    WITH base AS (
        ...
        WHERE g.cd_empfat = 1
          AND g.dt_transacao > '2025-01-01 00:00:00'::timestamp
    ...
    custos_mp AS (
        SELECT
            mp.cd_produtopa,
            sum(f_prd_valor_produto2(1::bigint, 1::bigint, 'C'::bpchar, 2::bigint, mp.cd_produtomp, NULL::timestamp) * mp.qt_consumo) AS customp
    ...
    WITH base AS (
        ...
        WHERE (
          ...
          AND t.dt_transacao >= '2025-01-01 00:00:00'::timestamp
    ...
    views = [
        ('vw_cmv_fab', sql_vw_cmv_fab),
        ('vw_cmv_lojas', sql_vw_cmv_lojas),
        ('vr_cmv_lojas', sql_vr_cmv_lojas),
    ]

    with conn.cursor() as cur:
        for view_name, sql in views:
            print(f"Atualizando {view_name}...", end=" ", flush=True)
            try:
                cur.execute(sql)
                print("OK")
