...
-- VERSAO COMPLETA: Janeiro a Julho de 2026
--
-- Grao da tabela:
--   1 linha por mes + loja + produto
--
-- ATENCAO: Esta query demora ~15-20 minutos por causa do f_prd_saldo_produto
...
WITH parametros AS (
    -- Gera todos os meses de 2026 (janeiro a julho)
    SELECT
        TO_CHAR(mes_date, 'YYYY-MM') AS mes,
        mes_date AS inicio_mes,
        (mes_date + INTERVAL '1 month')::date AS fim_mes,
        TO_CHAR(mes_date, 'YY.MM') || '%' AS lote_periodo
...
entradas_mes AS (
    ...
    JOIN vr_tra_transacao t
        ON i.dt_transacao >= p.inicio_mes
